...
class Lexer {
    public:
        ...
        tl::expected<std::reference_wrapper<std::vector<std::unique_ptr<Token>>>, Error> lexCharacterStream();
        static std::string printTokens(const std::vector<std::unique_ptr<Token>> &tokenStream);
    ...
    private:
        ...
        std::optional<TokenKind> getKwTokenKindFromString(const std::string &kw) const;
